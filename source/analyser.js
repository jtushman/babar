...
async function analyzeWithAI(content, metadata) {
  ...
  // Build schema from config and generate prompt
  const schema = buildSchemaFromConfig(config);
  const sections = generatePromptFromSchema(schema);

  // Replace variables in the prompt template
  const prompt = (config.promptTemplate || defaultConfig.promptTemplate)
  ...
  try {
    ...
    console.log('Model:', config.llm.ollama?.model || config.llm.openai?.model);
    console.log('Endpoint:', config.llm.ollama?.endpoint || 'OpenAI');
    console.log('\n');
    ...
    for (const [key, _field] of Object.entries(schema.shape || {})) {
    ...
    // If no schema or failed to parse structured response, use raw completion
    if (!Object.keys(schema.shape || {}).length) {
      console.log('Failed to parse structured response, attempting to format raw response');
      analysis.push('\n' + completion);
    }
  ...
  try {
    ...
    if (childrenAimd.length > 0) {
      const childSummaries = await Promise.all(
        childrenAimd.map(async (path) => {
          const content = await readFile(path, 'utf-8');
          return `Child Directory: ${relative(dirPath, path)}\n\n${content}`;
        })
      );
