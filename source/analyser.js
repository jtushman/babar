...
const analyzeDirectory = async (dirPath, files, childrenAimd = []) => {
	try {
		...
		console.log(
			`Found ${files.length} relevant files and ${childrenAimd.length} child summaries`,
		);
		...
		const validFiles = files.filter(file => {
			...
			return config.includeFiles.some(
				pattern =>
					...
					!config.excludePatterns.some(exclude =>
						minimatch(relativePath, exclude),
					),
			);
		});
		...
		if (analysis) {
			...
			console.log(
				`🐘 Successfully analyzed directory: ${relative(process.cwd(), dirPath)}`,
			);
		}
...
const processDirectoryNode = async (
	node,
	baseDir,
	onProgress,
	processedCount,
	...
	if (node.files.length > 0) {
		...
		try {
			...
			needsAnalysis = node.files.some(
				file => file.stats.mtime > aimdStats.mtime,
			);
		} catch (error) {
...
export const processDirectory = async (directory, onProgress) => {
	...
	for (const depth of depths) {
		...
		await Promise.all(
			nodes.map(node =>
				processDirectoryNode(node, directory, onProgress, processedCount),
			),
		);
	}
